...
public class TriLaserController : EnemyController {
    public float fireTime = 3f, laserAngle = 120f, mainWidth = .5f, sideWidths = .3f, mainGuideWidth = .08f, sideGuideWidths = .05f;
    ...
    public override IEnumerator FireSequence() {
        ...
        while (Time.time - startTime < cooldown / 2) {
            ...
            leftAngle = delta;
            rightAngle = -delta;
            leftContainer.transform.localRotation = Quaternion.Euler(new Vector3(0f, leftAngle, 0f));
            rightContainer.transform.localRotation = Quaternion.Euler(new Vector3(0f, rightAngle, 0f));
    ...
    void GuideOn(float percentage) {
        ...
        Vector3 rightDirection = ( right.transform.position - transform.position ).normalized;
        Vector3 leftDirection = ( left.transform.position - transform.position ).normalized;
        ...
        bool leftHit = Physics.Raycast(left.transform.position, leftDirection, out leftHitInfo, 1000f);
        if (leftHit) {
        ...
        bool rightHit = Physics.Raycast(right.transform.position, rightDirection, out rightHitInfo, 1000f);
        if (rightHit) {
